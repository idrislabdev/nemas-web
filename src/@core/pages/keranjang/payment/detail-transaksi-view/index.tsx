...
const KeranjangPaymentDetailTransaksiView = (props: {
    ...
    const processPayment = async () => {
        ...
        axiosInstance.post("/orders/fix/order/create/", payload)
        .then((response) => {
            ...
            <div className='detail-transaksi-section'>
                ...
                <div className='main-container'>
                    <div className='header-area'>
                        ...
                        <a onClick={() => setView('cart')} className='cursor-pointer'><span className='my-icon icon-sm'><ChevronLeft /></span><span className='text'>Kembali</span></a>
                    </div>
                    <div className='body-area'>
                        <div className='detail'>
                            ...
                            <div className='sub-detail total'>
                                <label>Total</label>
                                <p>Rp{formatterNumber(parseInt(order.order_amount) + parseInt(order.order_admin_amount) + parseInt(order.order_tracking_total) + parseInt(order.order_tracking_insurance))}</p>
                            </div>
                        </div>
                        <div className={`button-voucher-payment`}>
                            <button onClick={() => setIsModalPayment(true)}>
                                <span className='label-info'>
                                    ...
                                    {payment.method_id && `${payment.method_name} - ${payment.va_bank}` }
                                </span>
                        ...
                        <div className='confirmation-area'>
                            <Checkbox checked={checked} onChange={onChange} disabled={!payment.method_id} >Dengan ini saya menyetujui Syarat dan Ketentuan yang berlaku</Checkbox>
                            <button className='confirmation-button' onClick={() => checkPin()} disabled={checked ? false : true}>Proses Pembayaran</button>
                        </div>
                    </div>
                </div>
            </div>
