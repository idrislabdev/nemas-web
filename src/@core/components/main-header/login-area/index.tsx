...
const LoginArea = (props: {
  ...
  useEffect(() => {
    ...
    function handleClick(event: any) {
      ...
      if (
        dropdownuser.current &&
        !dropdownuser.current.contains(event.target)
      ) {
        setShowMenu(false);
      }
    }
  ...
  useEffect(() => {
    if (token != '') {
      axiosInstance
        .get(`orders/fix/cart/detail/?order_type=buy&offset=0&limit=100`)
        .then((response) => {
  ...
  return (
    <div
      className={`login-area ${userLogin.name ? 'items-center' : ''}`}
      ref={dropdownuser}
    >
      {userLogin.name && (
        <Link href={`/keranjang`} className="cart-notif">
          <span>
            <ShoppingCart01 />
          </span>
          {globals.cartCount > 0 && (
