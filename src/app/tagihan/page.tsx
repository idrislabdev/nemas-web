...
import DaftarTagihanPageWrapper from '@/@core/pages/tagihan';

const DaftarTagihanPage = () => {
  const cookieStore = cookies();
  const userLogin: IUserLogin = cookieStore.get('user')
    ? JSON.parse(cookieStore.get('user')?.value || '{}')
    : {}; //JSON.parse(getCookie('user') || "{}")
  const userProps: IUserProp = cookieStore.get('user_prop')
    ? JSON.parse(cookieStore.get('user_prop')?.value || '{}')
    : {};
  ...
  return (
    <GlobalsProvider>
      ...
      <DaftarTagihanPageWrapper userLogin={userLogin} />
      <Footer />
    </GlobalsProvider>
  );
};
